        ...
        try:
            ...
            if isinstance(conf_int, np.ndarray):
                conf_int = pd.DataFrame(
                    conf_int, columns=["lower", "upper"], index=data.index
                )
            ...
            if isinstance(conf_int, pd.DataFrame):
                lower_col = next(
                    (
                        ...
                        if str(col).lower().startswith("lower")
                    ),
                    conf_int.columns[0],
                )
                upper_col = next(
                    (
                        col
                        for col in conf_int.columns
                        if str(col).lower().startswith("upper")
                    ),
                    conf_int.columns[1],
                )
            ...
            for date, pred, lower, upper in zip(
                data.index, mean_forecast, lower_vals, upper_vals
            ):
                results.append(
                    {
            ...
            if len(results) > 1 and results[0]["prediction"] != 0:
                evolution_annuelle = (
                    100
                    * (results[-1]["prediction"] - results[0]["prediction"])
