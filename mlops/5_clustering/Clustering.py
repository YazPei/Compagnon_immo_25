...
def safe_emit(df_full: pd.DataFrame, out_cluster_csv: Path, out_st_csv: Path) -> None:
    """Écrit des sorties minimales valides si le clustering échoue."""
    ...
    # copies data/ pour stages suivants
...
def run_clustering_pipeline(input_path: str, output_path: str) -> None:
    ...
    with ctx:
        ...
        # Code postal (spatial si possible)
        if HAS_GPD and geo_file.exists():
            pass
        else:
        ...
        cluster_input = (agg.rename(columns={"prix_m2_vente":"prix_m2_mean"})
        ...
        if X.empty:
            if SAFE:
                print("[WARN] Features vides → SAFE MODE")
                return safe_emit(df, out_cluster_csv, out_st_csv)
            raise ValueError("Features vides après nettoyage.")

        # elbow + kmeans
        try:
            ks = list(range(2,9)); wcss=[]; sils=[]; models=[]
            ...
            if SAFE:
                print(f"[WARN] KMeans/elbow failed: {e}")
                return safe_emit(df, out_cluster_csv, out_st_csv)
            raise
        ...
        if HAS_MLFLOW:
            mlflow.log_artifact(str(out_st_csv)); mlflow.log_artifact(str(out_cluster_csv))
