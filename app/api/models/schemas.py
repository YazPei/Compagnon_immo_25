...
class QuestionnaireRequest(BaseModel):
    ...
    adresse: str = Field(..., description="Adresse complète du bien (numéro, rue, etc.)")
    code_postal: str = Field(..., description="Code postal du bien")
    ville: str = Field(..., description="Ville du bien")
    annee_construction: Optional[int] = Field(None, description="Année de construction (optionnel)")
    ...
    surface_terrain: Optional[float] = Field(0, description="Surface du terrain (optionnel, maisons)")
    ...
    eau: Optional[int] = Field(0, description="Nombre de points d'eau (salle d'eau, douche, etc.)")
    etage: Optional[int] = Field(0, description="Étage du bien (optionnel)")
    # Champs avancés pour le cluster ou le type de transaction
    typedetransaction: Optional[str] = Field(None, description="Type de transaction (vp, v, pi, etc.)")
    forced_cluster: Optional[int] = Field(None, description="Forcer un cluster SARIMAX (optionnel)") 