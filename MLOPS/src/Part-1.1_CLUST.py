...
test_cluster["codePostal_recons"] = test_cluster["zone_mixte"].apply(get_code_postal_final)
test_cluster.drop(columns=["zone_mixte"], inplace=True)

# ── 3. Fusion des features agrégées ──
test_cluster = test_cluster.merge(
...
missing = set(features) - set(test_cluster.columns)
if missing:
    raise ValueError(f"Il manque ces colonnes dans test_cluster avant clustering : {missing}")
...
display(test_cluster["cluster"].value_counts())
