...
@click.option(
    "--tuner",
    type=click.Choice(["none", "random"], case_sensitive=False),
    default="none",
    show_default=True,
...
@click.option(
    "--mem-mode",
    type=click.Choice(["auto", "row", "col"], case_sensitive=False),
    default="auto",
    show_default=True,
...
def main(encoded_folder, experiment, tuner, n_iter, cv, random_state, mem_mode):
    ...
    assert set(X_train.columns) == set(
        X_test.columns
    ), "Colonnes train/test encodées différentes."
    ...
    with mlflow.start_run(run_name=f"train_lgbm_{tuner}"):
        ...
        try:
            ...
            print(
                f"Score R² moyen (CV) : {np.mean(cv_scores):.4f} ± {np.std(cv_scores):.4f}"
            )
