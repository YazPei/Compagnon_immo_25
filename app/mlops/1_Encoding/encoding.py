...
def add_geo_columns_if_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if lat is None or lon is None:
        print(
            "⚠️  INFO: Impossible d'inférer les colonnes géo (lat/lon/geometry introuvables). "
    ...
    df = df.copy()
    if "x_geo" not in df.columns:
        df["x_geo"] = x
    if "y_geo" not in df.columns:
        df["y_geo"] = y
    if "z_geo" not in df.columns:
        df["z_geo"] = z
...
def _year_bucketize_array(X):
    ...
    labels = pd.cut(
        s_num, bins=_BINS, labels=_LABELS_ASC, right=True, include_lowest=True
    ).astype(object)
...
def build_preprocessor_dynamic(X_fit: pd.DataFrame) -> ColumnTransformer:
    ...
    transformers = []
    if present_ord:
        transformers.append(("ord", ordinal_pipeline, present_ord))
    if present_ohe:
        transformers.append(("ohe", onehot_pipeline, present_ohe))
    if present_year:
        transformers.append(("year", year_rank_pipeline, present_year))
    if present_tgt:
        transformers.append(("tar", target_pipeline, present_tgt))
    if present_num:
        transformers.append(("num", numeric_pipeline, present_num))
    if present_geo:
        transformers.append(("geo", geo_pipeline, present_geo))
...
def get_feature_names(preprocessor: ColumnTransformer, X_fit: pd.DataFrame) -> list:
    ...
    for name, trans, cols in preprocessor.transformers_:
        ...
        cols_list = (
            ...
            else X_fit.columns[cols].tolist()
        )
        if name == "ohe":
...
@click.option(
    "--data-path",
    default="data/df_cluster.csv",
    show_default=True,
    help="CSV brut d'entrée.",
)
@click.option(
    "--output",
    default="data/encoded",
    show_default=True,
    help="Dossier de sortie des encodages.",
)
@click.option(
    "--experiment",
    default="regression_pipeline",
    show_default=True,
...
def main(data_path, output, experiment, random_state):
    ...
    with mlflow.start_run(run_name="encode_only"):
        ...
        pd.DataFrame(X_train_enc, columns=encoded_columns).to_csv(
            paths["X_train"], sep=";", index=False
        )
        pd.DataFrame({TARGET: y_train}).to_csv(paths["y_train"], sep=";", index=False)
        pd.DataFrame(X_test_enc, columns=encoded_columns).to_csv(
            paths["X_test"], sep=";", index=False
        )
