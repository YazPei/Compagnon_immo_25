...
def load_and_clean_data(folder_path1, folder_path2):
    input_file1 = os.path.join(folder_path1, 'df_sample.csv')
...
def merge_macro_indicators(df_sales_clean, df_dvf, df_ips):
    ...
    for col in ['avg_purchase_price_m2', 'avg_rent_price_m2', 'rental_yield_pct', 'IPS_primaire']:
        source = df_dvf_small if 'avg' in col or 'yield' in col else df_ips_small
        df_sales_clean[col] = df_sales_clean['INSEE_COM'].map(dict(zip(source['INSEE_COM'], source[col])))
...
@click.command()
@click.option('--folder-path1', type=click.Path(exists=True), prompt='📂 Chemin vers les données ventes')
...
def main(folder_path1, folder_path2, output_folder):
    ...
    with mlflow.start_run(run_name="fusion_geo_dvf"):
        ...
        mlflow.log_metric("nb_missing_ips_initial", df_sales_clean['IPS_primaire'].isna().sum() + nb_imputed)
        mlflow.log_metric("nb_imputed_ips", nb_imputed)
        mlflow.log_metric("nb_final_missing_ips", df_sales_clean['IPS_primaire'].isna().sum())
        mlflow.log_param("output_folder", output_folder)

        # === Export final ===
        Path(output_folder).mkdir(parents=True, exist_ok=True)
        output_path = Path(output_folder) / "df_sales_clean.csv"
