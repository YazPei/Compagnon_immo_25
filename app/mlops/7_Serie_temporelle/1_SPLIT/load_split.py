...
def process_chunk(
    chunk: pd.DataFrame, pcodes_gdf: gpd.GeoDataFrame, debug: bool = False
) -> pd.DataFrame:
    ...
    if debug:
        logging.info(
            f"[debug] within: cp_col={cp_col}, hit={(None if cp_col is None else (~joined[cp_col].isna()).sum())}"
        )
...
def enrich_and_split():
    ...
    with MLFLOW.start_run(run_name="full_encoding"):
        ...
        in_metro_valid = (df_valid["lat"].between(41.0, 51.5)) & (
            df_valid["lon"].between(-5.5, 9.8)
        )
        ...
        if results:
            df_joined = pd.concat(results, ignore_index=True).drop_duplicates(
                "orig_index"
            )
        else:
        ...
        # Merge sécurisé
        cp_like_left = [
            c for c in df.columns if c.lower().replace("_", "") == "codepostal"
        ]
        if cp_like_left:
        ...
        if "codePostal_cpjoin" in df.columns:
            if "codePostal" in df.columns:
                df["codePostal"] = (
                    df["codePostal"]
                    .astype("string")
                    .fillna(df["codePostal_cpjoin"].astype("string"))
                )
        ...
        if "cluster" not in df.columns:
            map_path = next(
                (p for p in CLUSTER_MAPPING_CANDIDATES if os.path.exists(p)), None
            )
            if map_path is None:
                df["cluster"] = 0
            else:
                ...
                merged = False
                if (
                    "INSEE_COM" in df.columns
                    and "INSEE_COM" in map_df.columns
                    and "cluster" in map_df.columns
                ):
                    df["INSEE_COM"] = df["INSEE_COM"].astype(str).str.zfill(5)
                    map_df["INSEE_COM"] = map_df["INSEE_COM"].astype(str).str.zfill(5)
                    df = df.merge(
                        map_df[["INSEE_COM", "cluster"]].drop_duplicates(),
                        on="INSEE_COM",
                        how="left",
                    )
                    merged = True
                if (
                    not merged
                    and "codePostal" in map_df.columns
                    and "cluster" in map_df.columns
                ):
                    map_df["codePostal"] = map_df["codePostal"].astype(str).str.zfill(5)
                    df = df.merge(
                        map_df[["codePostal", "cluster"]].drop_duplicates(),
                        on="codePostal",
                        how="left",
                    )
                    merged = True
        ...
        def add_geo_coords(df_):
            ...
            return df_.drop(
                columns=["mapCoordonneesLatitude", "mapCoordonneesLongitude"]
            )
        ...
        agg_train = agg_train.merge(
            taux_m, left_on="date", right_on="month", how="left"
        ).drop(columns=["month"])
        ...
        train_q12_path = os.path.join(
            OUTPUT_FOLDER, f"train_periodique_q12{SUFFIX}.csv"
        )
