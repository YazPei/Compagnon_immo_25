...
from compagnon_immo.api.utils.exceptions import CompagnionImmoException
...
async def http_exception_handler(
    ...
    logger.error(
        f"HTTP Exception: {exc.detail} - URL: {request.url} - Status: {exc.status_code}"
    )
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(f"Unhandled exception: {str(exc)} - URL: {request.url}", exc_info=True)
