...
default_args = {
    'owner': 'airflow',
    'start_date': pendulum.datetime(2023, 10, 1, tz='UTC').naive(),  # Conversion en datetime
    'retries': 3,
}
...
def train_v1(**ctx):
    """
    Entraîne et logue un modèle de régression linéaire dans MLflow.
    """
    # Charger les données (exemple avec sklearn et make_regression)
    ...
    with mlflow.start_run(run_name="train_v1") as run:
        ...
        # Log de la métrique
        mlflow.log_metric(TARGET_METRIC, model.score(X, y))
        
        # Stocke le run_id pour les tâches suivantes
        ctx["ti"].xcom_push(key="run_id", value=run_id)
        print(f"[train_v1] run_id={run_id}")

def register_v1(**ctx):
    """Crée une version du modèle dans le registre à partir du run_id du modèle entraîné."""
    mlflow.set_tracking_uri(MLFLOW_URI)
    client = MlflowClient()
...
def compare_and_promote(**ctx):
    ...
    # récupérer la métrique du candidat
    ...
    # récupérer la métrique du modèle en production
    try:
