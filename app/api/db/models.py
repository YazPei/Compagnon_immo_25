...
class Estimation(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    property_data = Column(Text, nullable=False)  # JSON des caractéristiques
    estimated_price = Column(Float, nullable=False)
    confidence_score = Column(Float)
    
    # RÉFÉRENCE MLflow au lieu de duplication
    mlflow_run_id = Column(String(255), index=True)  # ID du run MLflow
    model_name = Column(String(100))  # Nom du modèle utilisé
