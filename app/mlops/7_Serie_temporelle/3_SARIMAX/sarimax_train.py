...
warnings.filterwarnings(
    "ignore", category=ValueWarning, message="No frequency information was provided.*"
)
...
def filter_exog_by_corr(
    y: pd.Series, exog_df: pd.DataFrame, threshold=0.05
) -> pd.DataFrame:
...
def apply_lag_if_correlated(
    df: pd.DataFrame, target="prix_m2_vente", threshold=0.3
...
def get_diff_order(
    series: pd.Series, seasonal=False, s=12, max_d=2, cutoff=0.05
) -> int:
...
def prepare_monthly_index(
    y: pd.Series, exog: Optional[pd.DataFrame] = None, min_frac_non_nan: float = 0.9
):
    """
    ...
    if exog is not None and not exog.empty:
        ...
        ex_scaled = pd.DataFrame(
            scaler.fit_transform(ex.values), index=ex.index, columns=ex.columns
        )

    return (
        y.asfreq("MS"),
        (ex_scaled.asfreq("MS") if ex_scaled is not None else None),
        scaler,
        full_idx,
    )
...
def fit_sarimax_robust(
    y, exog, order, seasonal_order, trend="c", maxiter=100, start_params=None
):
...
def evaluate_model(model_result) -> Dict[str, float]:
    ...
    try:
        ljung_pval = acorr_ljungbox(resid, lags=[lag], return_df=True)[
            "lb_pvalue"
        ].iloc[0]
    except Exception:
        ljung_pval = np.nan
...
def try_sarimax_grid(
    ...
    for p, q in itertools.product(range(max_p + 1), range(max_q + 1)):
        for P, D_seas, Q, s_val in get_valid_seasonal_orders(
            D, s, max_P=max_p, max_Q=max_q
        ):
            for exog_subset in exog_subsets:
                ...
                try:
                    ...
                    if (
                        metrics["ljung_pvalue"] > 0.05
                        and metrics["normal_pvalue"] > 0.05
                        and (best_model is None or metrics["aic"] < best_metrics["aic"])
                    ):
                        best_model, best_metrics, best_params = (
                            result,
                            metrics,
                            {
                                "order": (p, d, q),
...
def fallback_prophet(df_cluster, cluster_id, output_folder: str, suffix: str = ""):
    ...
    if isinstance(df_cluster, pd.Series):
        ...
        if "prix_m2_vente" not in df_cluster.columns:
            raise ValueError(
                "fallback_prophet: colonne 'prix_m2_vente' absente du DataFrame."
            )
        y = pd.to_numeric(df_cluster["prix_m2_vente"], errors="coerce")
        idx = df_cluster.index
    else:
        raise TypeError(
            "fallback_prophet: df_cluster doit être une Series ou un DataFrame."
        )
    ...
    prophet_df = prophet_df.drop_duplicates(subset="ds", keep="last").reset_index(
        drop=True
    )

    if prophet_df.empty:
        raise ValueError(
            "fallback_prophet: aucune donnée valide (ds,y) pour Prophet après nettoyage."
        )
    ...
    if suffix:
        path_suffix = os.path.join(
            output_folder, f"cluster_{cluster_id}_prophet{suffix}.pkl"
        )
...
def train_all_clusters(
    ...
    if missing:
        raise ValueError(
            f"Colonnes manquantes dans les fichiers periodiques: {missing}"
        )
    ...
    with mlflow.start_run(run_name="sarimax_all_clusters"):
        for cluster_id in sorted(full["cluster"].dropna().unique()):
            ...
            exog_vars = [
                c for c in dfc.columns if c not in ["prix_m2_vente", "cluster"]
            ]
            ...
            with mlflow.start_run(run_name=f"cluster_{cluster_id}", nested=True):
                ...
                if scaler_exog is not None:
                    scaler_path = os.path.join(
                        output_folder, f"cluster_{cluster_id}_exog_scaler.pkl"
                    )
                ...
                if X_m2 is None or X_m2.empty:
                    ...
                    fallback_prophet(
                        (
                            pd.concat([y_m.rename("prix_m2_vente"), X_m], axis=1)
                            if X_m is not None
                            else y_m.to_frame("prix_m2_vente")
                        ),
                        cluster_id,
                        output_folder,
                        suffix=suffix,
                ...
                if best_model:
                    path_main = os.path.join(
                        output_folder, f"cluster_{cluster_id}_sarimax.pkl"
                    )
                    ...
                    if suffix:
                        path_suffix = os.path.join(
                            output_folder, f"cluster_{cluster_id}_sarimax{suffix}.pkl"
                        )
                    ...
                    print(
                        f"✅ [Cluster {cluster_id}] SARIMAX validé (opt={best_params.get('optimizer_used')}) et sauvegardé."
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Suffixe à ajouter (copie supplémentaire)",
    )
    parser.add_argument(
        "--save-split",
        action="store_true",
        help="Sauvegarder les fichiers train/test par cluster",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Top-k exogènes par corrélation à garder",
    )
    parser.add_argument(
        "--max-exog-comb-size",
        type=int,
        default=3,
        help="Taille max des combinaisons d'exogènes",
    )
    parser.add_argument(
        "--max-models",
        type=int,
        default=None,
        help="Nombre max de modèles testés (early stop)",
    )
    # mode test - sampling
    parser.add_argument(
        "--sample-last",
