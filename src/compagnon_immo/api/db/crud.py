...
from compagnon_immo.api.db.models import Estimation, Property, User

try:
    from compagnon_immo.api.db.schemas import EstimationCreate, PropertyCreate
except ImportError:
...
try:
    ...
    logger.warning("MLflow non disponible - " "fonctionnalités MLflow désactivées")
...
class PropertyCRUD:
    ...
    @staticmethod
    def get_by_filters(
        db: Session, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0
    ) -> List[Property]:
        ...
        if filters:
            ...
            if filters.get("min_surface"):
                query = query.filter(Property.surface >= filters["min_surface"])
            if filters.get("max_surface"):
                query = query.filter(Property.surface <= filters["max_surface"])
...
class EstimationCRUD:
        ...
        if MLFLOW_AVAILABLE:
            try:
                with mlflow.start_run():
                    ...
                    mlflow.log_param(
                        "surface", estimation_data.property_data.get("surface")
                    )
                    mlflow.log_param(
                        "postal_code", estimation_data.property_data.get("postal_code")
                    )
    ...
    def get_with_mlflow_details(
        ...
        return {"estimation": estimation, "mlflow_details": mlflow_details}
    ...
    def get_estimations(
        ...
        if filters:
            if filters.get("date_debut"):
                query = query.filter(Estimation.created_at >= filters["date_debut"])
            if filters.get("date_fin"):
                query = query.filter(Estimation.created_at <= filters["date_fin"])
            if filters.get("model_name"):
                query = query.filter(Estimation.model_name == filters["model_name"])
            if filters.get("min_price"):
                query = query.filter(Estimation.estimated_price >= filters["min_price"])
            if filters.get("max_price"):
                query = query.filter(Estimation.estimated_price <= filters["max_price"])

        return (
            query.order_by(desc(Estimation.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    @staticmethod
    def count_estimations(db: Session, filters: Optional[Dict[str, Any]] = None) -> int:
    ...
    @staticmethod
    def get_estimation_stats(db: Session, date_limite: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            median_query = (
                db.query(Estimation.estimated_price)
                .filter(Estimation.created_at >= date_limite)
...
def get_estimation_by_id(db: Session, estimation_id: str) -> Optional[Estimation]:
